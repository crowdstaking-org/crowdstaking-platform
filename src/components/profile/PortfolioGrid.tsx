...
export function PortfolioGrid({ walletAddress }: PortfolioGridProps) {
  ...
  if (!portfolio) {
    return <div className="text-gray-600 dark:text-gray-400">Could not load portfolio</div>
  }
      ...
      {portfolio.completedMissions && portfolio.completedMissions.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
            Completed Missions ({portfolio.completedMissions.length})
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {portfolio.completedMissions.map((mission: any) => (
              ...
              >
                <h3 className="font-bold text-gray-900 dark:text-white mb-2">{mission.title}</h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-3 line-clamp-2">{mission.description}</p>
      ...
      {(!portfolio.completedMissions || portfolio.completedMissions.length === 0) &&
        (!portfolio.foundedProjects || portfolio.foundedProjects.length === 0) && (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">No completed projects or missions yet</p>
          </div>
        )}
    </div>
  )
